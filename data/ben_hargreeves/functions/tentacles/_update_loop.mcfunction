...
execute at @s as @e[type=marker,tag=tent.node] if score $current tent.uuid = @s tent.uuid if score @s tent.node_id = $next_node tent.node_id run function ben_hargreeves:tentacles/_update_loop with storage ben_hargreeves:tent update
tag @s add temp
...
#execute facing entity @s feet run function ben_hargreeves:tentacles/_particle_ray
execute facing entity @s feet run function ben_hargreeves:tentacles/settings/ray_particles