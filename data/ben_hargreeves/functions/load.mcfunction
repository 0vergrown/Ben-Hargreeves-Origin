...
#define storage ben_hargreeves:tent for uses of macros in ben_hargreeves:tentacles namespace
...
function ben_hargreeves:tentacles/settings/main
...
execute store result storage ben_hargreeves:tent ray.step float 0.01 run scoreboard players get $RAY_STEP tent.uuid
execute store result storage ben_hargreeves:tent update.segment_length float 0.01 run scoreboard players get $SEGMENT_LENGTH tent.uuid
execute store result storage ben_hargreeves:tent choose_target.range float 0.01 run scoreboard players get $ATTACK_RANGE tent.uuid